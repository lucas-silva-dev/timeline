...
class timeline extends Component {
  ...
  getHour = date => {
    let t = new Date(date)
    let timeFormated = t.toLocaleTimeString('pt-BR', { hour: '2-digit', minute: '2-digit' })
  ...
  format = () => {
    ...
    data.map(item => {
      ...
      item.custom_data.map(item => {
        ...
        if (item.product_price !== undefined) obj.product_price = `${item.product_price},00`
